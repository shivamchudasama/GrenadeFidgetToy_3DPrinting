...
print("=== SLIDING ROD (WITHOUT DISC 27) FROM TOP (+Y = +70 down to 0) INTO BARREL ASSEMBLY ===")
for dy in range(70, -5, -2):
    rod_trans = rod_no_disc.translate([0, dy, 0])
    inter = rod_trans ^ barrel_rigid
    vol = inter.volume()
    status = f"CLASH ({vol:.2f} mm3)" if vol > 0.01 else "CLEAR (0 mm3)"
    print(f"  Delta Y = {dy:+3d} mm (rod bottom at Y={18.08+dy:5.1f}) : {status}")
