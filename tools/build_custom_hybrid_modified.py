...
# Distinct, vibrant, harmonious PBR color palette for all 36 parts
PART_COLORS = {
    ...
    # Compact Upper Station Clicker
    "08 - Internal Barrel (Compact Hybrid)": (206, 140, 90),           # Terracotta
    "12 - Internal Barrel Spring v1.1 (Upper Detent)": (98, 176, 140),  # Mint Green
    "13 - Internal Barrel Spring v1.1 (Upper Detent)": (196, 104, 133), # Berry Pink
    "14 - Internal Barrel Spring v1.1 (Upper Detent)": (126, 158, 190), # Steel Blue
    ...
    # 3-Part Full-Depth Rod & Cross-Locks
    ...
    # Updated Modified Head Components
    "09 - Rod Spring": (228, 169, 73),                  # Amber Gold
    "Custom Handle Left": (93, 150, 209),               # Slate Blue (Modified)
    "Custom Handle Right": (110, 132, 203),             # Cobalt Blue (Modified)
    "Custom Ring Spinner": (122, 181, 102),             # Olive Green (Modified)
    "Spinner Lever 05 - Gear": (219, 124, 171),         # Rose / Magenta
    "Spinner Lever 04 - Spring": (206, 140, 90),        # Terracotta Orange
    "Custom 16 Handle Lock (Neck)": (150, 120, 196),     # Purple (Relocated)
    "Custom 16 Handle Lock (Pod)": (184, 126, 152),      # Mauve (Relocated)
...
def build_modified_hybrid_items() -> list[tuple[str, trimesh.Trimesh, tuple[int, int, int]]]:
    """Assemble all 36 components using the updated modified head parts."""
    base = BCH.build_base()
    rod = BCH.build_rod()

    # Load updated head parts directly from Native_Waist_33_Click_Modified
    ...
    # Posed gear and spring from custom
    gear_p = custom.gear_posed()
    spring_p = custom.gear_spring_arm()

    head_raw = [
        (custom.HINGE_SPRING, rod_spring),
...
def compute_hybrid_exploded_displacements(items: list[tuple[str, trimesh.Trimesh, tuple[int, int, int]]]) -> np.ndarray:
    ...
    disp[idx["32 - Mid Shell P02"]] = [18.0, 0.0, 0.0]
    disp[idx["33 - Mid Shell P01"]] = [-18.0, 0.0, 0.0]
    disp[idx["20 - Mid Shell Spring"]] = [0.0, 0.0, 18.0]

    # Upper Station (displace upwards in +Y and radial)
    disp[idx["07 - Internal Barrel Cap"]] = [0.0, 12.0, 0.0]
    disp[idx["28 - Upper Shell Gear (32-Click Hybrid)"]] = [0.0, 22.0, 0.0]
    disp[idx["29 - Upper Shell Lock Ring"]] = [0.0, 16.0, 0.0]
    disp[idx["30 - Upper Shell Rotating Spring"]] = [0.0, 32.0, 0.0]
    disp[idx["27 - Upper Shell Top (Compact Chamber)"]] = [0.0, 42.0, 0.0]

    # Upper internal detent springs (spread radially)
    disp[idx["12 - Internal Barrel Spring v1.1 (Upper Detent)"]] = [0.0, 8.0, 14.0]
    disp[idx["13 - Internal Barrel Spring v1.1 (Upper Detent)"]] = [12.0, 8.0, -7.0]
    disp[idx["14 - Internal Barrel Spring v1.1 (Upper Detent)"]] = [-12.0, 8.0, -7.0]

    # Rod Members (axial + lateral spread)
    disp[idx["Custom Rod Right"]] = [-14.0, 15.0, 0.0]
    disp[idx["Custom Rod Middle"]] = [0.0, 25.0, 0.0]
    disp[idx["Custom Rod Left"]] = [14.0, 15.0, 0.0]
    disp[idx["Custom Rod Upper Right"]] = [8.0, 35.0, 0.0]
    disp[idx["Custom Rod Upper Left"]] = [-8.0, 35.0, 0.0]
    disp[idx["Spinner Lever 06 - Rod Lock"]] = [0.0, 25.0, 18.0]
    disp[idx["Spinner Lever 07 - Rod Lock"]] = [0.0, 25.0, -18.0]
    disp[idx["Custom Rod Upper Lock"]] = [0.0, 35.0, 14.0]
    disp[idx["Spinner Lever 08 - Rod Lock"]] = [0.0, 5.0, 0.0]

    # Head Components (lifted with the rod yoke and parted along X / Z)
    head_lift = 35.0
    disp[idx["09 - Rod Spring"]] = [0.0, head_lift + 10.0, 0.0]
    disp[idx["Custom Handle Left"]] = [-20.0, head_lift + 10.0, 0.0]
    disp[idx["Custom Handle Right"]] = [20.0, head_lift + 10.0, 0.0]
    disp[idx["Custom Ring Spinner"]] = [0.0, head_lift + 10.0, -25.0]
    disp[idx["Spinner Lever 05 - Gear"]] = [0.0, head_lift + 10.0, -15.0]
    disp[idx["Spinner Lever 04 - Spring"]] = [0.0, head_lift + 10.0, 0.0]
    disp[idx["Custom 16 Handle Lock (Neck)"]] = [35.0, head_lift + 10.0, 0.0]
    disp[idx["Custom 16 Handle Lock (Pod)"]] = [35.0, head_lift + 10.0, -20.0]
    disp[idx["15 - Handle Rotating Lock"]] = [35.0, head_lift + 10.0, 15.0]
