...
Directly inspired by the reference Spinner Fuse Grenade (`01 - Upper Shell.stl`), `18_27_Upper_Shell_Top` features a conical concave seating dish ($r_{\text{mouth}} = 12.00\text{ mm}$ at $Y = 74.65\text{ mm}$, $r_{\text{floor}} = 9.20\text{ mm}$ at $Y = 68.00\text{ mm}$), providing a wide $3.51\text{ mm}$ flat annular rim ($303.23\text{ mm}^2$ area) for reliable first-layer build plate adhesion during 3D printing.
