...
- `tools/` — Python toolchain:
  ...
  - `build_modified_rotating_spring.py` — builds the refined Upper Shell Rotating Spring (`21_30`) with extended low-angle ski-tip glide ramp on Flank B (+3.5 mm, $R = 0.75\text{ mm}$ curl) eliminating counter-clockwise rotation wedging/jerk. Exports Option 1 (Balanced Comfort, $\Delta t = 0.25\text{ mm}$, $k = 3.34\text{ N/mm}$, $539\text{ gf}$) as the definitive production spring (Option 2 retired per analysis).
...
- **Concave Upper Seating & Deeply Nested Lever (`v1.3`)**: Inspired by Spinner Fuse `01 - Upper Shell`, `18_27_Upper_Shell_Top` carries a conical concave dish ($r_{\text{mouth}} = 12.00\text{ mm}$, $r_{\text{floor}} = 9.20\text{ mm}$, top plane $Y = 74.65\text{ mm}$), providing a robust $3.51\text{ mm}$ wide flat top contact rim with $303.23\text{ mm}^2$ surface area for bed adhesion during 3D printing. Central rod hinge yoke lowered by $\Delta Y = -11.46\text{ mm}$ ($Y_{\text{pin}} = 81.00\text{ mm}$, ear tops $Y = 88.16\text{ mm}$), allowing the folding lever cheeks to nest deeply inside the dish with running clearance across all 4 detent angles ($0^\circ, 30^\circ, 60^\circ, 90^\circ$). `21_30_Upper_Shell_Rotating_Spring` idle guide sleeve is trimmed at $Y = 67.20\text{ mm}$ (3-arm clicker intact), side clamps trimmed to match ($5.20\text{ mm}$ hex engagement preserved), and hinge detent spring compacted to $14.69\text{ mm}$ height ($1.41\text{ mm}$ clearance above cross-key 06).
- **Refined Upper Rotating Spring with Ski-Tip Glide Ramp (`v1.3`)**: To eliminate counter-clockwise rotation wedging/spragging and release jerk against the 36 teeth of `18_27_Upper_Shell_Top`, Flank B after the 120° apex turn is elongated by +3.5 mm into a gentle low-angle glide ramp down to $R = 15.55\text{ mm}$ at $\theta = 18.5^\circ$, terminating in an inward-curled rounded ski-tip ($R = 0.75\text{ mm}$). Built by `tools/build_modified_rotating_spring.py`. Option 1 Balanced Comfort ($\Delta t = 0.25\text{ mm}$, $k = 3.34\text{ N/mm}$, $539\text{ gf}$ peak snap force, $151\text{ gf}$ seat hold, $0.90\%$ peak strain) is selected and kept as the definitive production spring for optimal fatigue life and finger comfort; Option 2 (Firm Click) was retired per analysis.
...
python tools/build_modified_rotating_spring.py # only if the rotary clicker spring changed
