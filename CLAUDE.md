...
**`Hybrid_Grenade_v1.2/` is the current package; `Hybrid_Grenade_v1.1/` is frozen.** They differ in the axial rod detent and in what that layout cost — the three barrel pins and one cap key — see **The custom hybrid build**. Both hold the same folder layout:
...
- `tools/` — Python toolchain:
  ...
  - `build_rod_detent.py` — builds the four rod detent springs, fills the barrel's three pin channels, cuts its four spring slots and removes one cap key. `--dry-run` reports without writing, and its `insertion_clear()` check is the one that would have caught v1.2's un-installable follower.
...
- **Tactical Internal Spine & Upper Station**: `08 - Internal Barrel` capped by `11_Custom_Internal_Barrel_Cap`, carrying the top assembly (`27`, `28`, `29`, `30`). Upstream, 3 pins (`09/10/11`) held that assembly down; they sat **1.47–3.33 mm clear of the rod** and were *not* part of the axial detent, whatever older notes say — nor is `09_Custom_Mid_Shell_Spring_33`, which clears the rod by 1.158 mm and is rotary only. In v1.2 the pins are gone; see the next bullet but one.
- **Axial Rod Detent** (`v1.2`): **four arm springs at azimuth 0°/90°/180°/270°**. Each is the Spinner Fuse `11 - Middle Spring` cut in half at the bridge and its arm **transplanted outline-for-outline** — mapped into the slot at 0.53 radial / 0.74 axial scale, keeping the long slanted leg, the fold to an outer rail, the U-turn and the hook back to the nose. Only the ends are ours: a rigid foot where `11` had its bridge, carrying the *extended notch*, and a nose cut for this rack. The rod's rack is a **turned ring, not flat faces**, so any azimuth works. Rate **0.631 N/mm per arm at 0.831 %/mm** against `11`'s 1.207/0.714 and v1.1's 2.951/1.278; swept force **2.26 N peak, 0.45 N held at every seat, 0% dead band** against `11`'s own 2.00 N and v1.1's 5.53 N with a 12% dead band. Slots are the stock section 0.30 mm wider, stopping at r 12.30 rather than 14.55, because above y 54 the barrel is a narrow cylinder (13.95 in the troughs) and only its three lobes at 90°/210°/330° carried the deeper stock slots. Barrel 19365 → 19022 mm³, thinnest wall left 1.50 mm, outer surface untouched. **v1.1 and v1.2 barrels and caps are not interchangeable.**
- **Exactly four slots, and what that cost** (`v1.2`): the barrel now carries four slots and nothing else. The three pin channels at 30°/150°/270° are filled — four slots 90° apart cannot miss three channels 120° apart — and so are the three stock follower slots at 90°/210°/330°, which otherwise leave a hybrid profile where one overlaps a new slot. `09/10/11 - Internal Barrel Pin` are gone, and with them the **only** thing holding `27 - Upper Shell Top` down: lifting it now meets 0.000 mm³ of interference at every height. Rotation is unaffected (the barrel keys the shell with its own lobes). **Designing a replacement is open work.** The cap loses all three keys with the slots they sat in and becomes a plain disc; its underside at y 63.205 is what now traps the springs.
