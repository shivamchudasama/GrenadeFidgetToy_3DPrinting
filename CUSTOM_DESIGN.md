...
2. **Spinner Fuse Concave Upper Seating & Wide 303 mm² Print Bed Contact**: Conical concave dish ($r_{\text{mouth}} = 12.00\text{ mm}$ at $Y = 74.65\text{ mm}$, $r_{\text{floor}} = 9.20\text{ mm}$ at $Y = 68.00\text{ mm}$) on `18_27_Upper_Shell_Top`, lowering the hinge axis from $Y = 92.46\text{ mm}$ down to $Y = 81.00\text{ mm}$ ($\Delta Y = -11.46\text{ mm}$) so the folding lever cheeks nest deeply inside the shell while preserving a $3.51\text{ mm}$ wide flat top contact rim with $303.23\text{ mm}^2$ surface area for 100% reliable build plate adhesion.
...
4. **Spinner Fuse Concave Upper Seating Arrangement**: Conical concave dish on `18_27_Upper_Shell_Top` ($r_{\text{mouth}} = 12.00\text{ mm}$, $r_{\text{floor}} = 9.20\text{ mm}$, flat top contact area $303.23\text{ mm}^2$) with the folding head assembly lowered by $\Delta Y = -11.46\text{ mm}$ ($Y_{\text{hinge}} = 81.00\text{ mm}$), eliminating the exposed neck gap and nesting the handle cheeks below the outer rim.
...
| **06. Hex Tactical** | 2-Piece (39 Honeycomb Hex Windows, ⌀5.70 mm / r=2.85 mm) | **34.80 mm** (Outer)<br>**33.80 mm** (Inner) | **40.00 mm** | 42.12 mm | `08_Mid_Shell_Option_06_Hex_Tactical_Outer.stl`<br>`07_32_Mid_Shell_P02_Ratchet.stl` |
